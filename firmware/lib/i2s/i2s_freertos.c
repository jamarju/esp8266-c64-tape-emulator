	...
	*/
	for (bckdiv=2; bckdiv<64; bckdiv++) {
		for (clkmdiv=5; clkmdiv<64; clkmdiv++) {
			tstfreq=BASEFREQ/(bckdiv*clkmdiv);
	...
	printf("ReqRate %d MDiv %d BckDiv %d  Frq %d\n", 
		rate, bestclkmdiv, bestbckdiv, (int)(BASEFREQ/(bestbckdiv*bestclkmdiv)));
	...
	SET_PERI_REG_MASK(I2SCONF, I2S_RIGHT_FIRST|I2S_MSB_RIGHT|I2S_RECE_SLAVE_MOD|
						I2S_RECE_MSB_SHIFT|I2S_TRANS_MSB_SHIFT|
						((0)<<I2S_BITS_MOD_S)|	// extra padding bits every 16 bits
...
//Data is pushed in groups of 32 bits, MSb first.
